...
function createSystemMessage(prompt: string, userMessage: string): string {
  ...
  const isAddressQuery = ['address', 'location', 'office', 'where'].some(term =>
    userMessage.toLowerCase().includes(term)
  );

  if (isAddressQuery) {
    return `You are an assistant for ${businessName}. The location is ${location}. Answer only with the location from the prompt, concisely (e.g., "${location}").`;
  }
  return `You are an assistant for ${businessName}. Use details from this prompt: "${prompt}". Answer the user's question concisely, using only prompt details, in under 50 words. Avoid adding unprompted information.`;
}
