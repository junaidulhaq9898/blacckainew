...
function generateSmartFallback(messageText: string, prompt?: string, history: any[] = []): string {
  // Force Delight Brush branding for this automation
  const basePrompt = "Hello! I’m with Delight Brush Industries, your paint brush experts.";
  const lastUserMsg = history.filter(m => m.role === 'user').pop()?.content || "";
  let response = basePrompt;

  // Contextual responses
  if (messageText.toLowerCase().includes("business name") || lastUserMsg.toLowerCase().includes("business name")) {
    response += " Our business is Delight Brush Industries, specializing in all types of paint brushes. How can I assist you today?";
  } else if (messageText.toLowerCase().includes("pro plan") || lastUserMsg.toLowerCase().includes("pro plan")) {
    response += " We don’t offer plans directly, but I can tell you about our paint brushes! May I have your contact number for our sales team to assist?";
  } else if (messageText.toLowerCase().includes("price") || lastUserMsg.toLowerCase().includes("price")) {
    response += " I’d be happy to help with pricing. Could you share your contact number so our team can provide details?";
  } else {
    response += " How can I assist you with our high-quality paint brushes today?";
  }

  return response.slice(0, 2000); // Ensure under Instagram’s limit
}
      ...
      if (automation.User?.subscription?.plan === 'PRO') {
        try {
          ...
          const aiPrompt = automation.listener?.prompt
            ? `You are an assistant for a specific business. ${automation.listener.prompt} Answer strictly as the business representative, using only the business context provided, and do not improvise details or mention being an AI, model, or anything unrelated.`
          ...
          if (smart_ai_message?.choices?.[0]?.message?.content) {
            ...
            const fallbackResponse = generateSmartFallback(commentText, automation.listener?.prompt, limitedHistory);
      ...
      if (automation.User?.subscription?.plan === 'PRO') {
        try {
          ...
          if (smart_ai_message?.choices?.[0]?.message?.content) {
            ...
            console.error("❌ No content in AI response:", smart_ai_message);
            const fallbackResponse = generateSmartFallback(messageText, automation.listener?.prompt, limitedHistory);
            console.log("📤 Sending smart fallback DM:", fallbackResponse);
