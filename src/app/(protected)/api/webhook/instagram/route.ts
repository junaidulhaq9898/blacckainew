...
export async function POST(req: NextRequest) {
  try {
      ...
      if (matcher?.automationId) {
        ...
        automation = await client.automation.create({
          data: {
            ...
            listener: {
              create: {
                prompt: "You are a helpful assistant for my business, WebProdigies.",
                commentReply: "ok",
                listener: "MESSAGE",
              },
            },
    ...
    const prompt = automation.listener?.prompt || "You are a helpful assistant for my business, WebProdigies.";
    ...
    if (plan === 'PRO') {
      try {
        ...
        if (!aiResponse || aiResponse.trim() === "") {
          ...
          aiResponse = generateSmartFallback();
        }
