...
function generateSmartFallback(messageText: string, prompt?: string, history: any[] = []): string {
  const basePrompt = prompt?.includes("Delight Brush Industries")
    ? "Hello! I’m with Delight Brush Industries, your paint brush experts."
    : "Hi there! I’m here to help with your business needs.";
  
  // Check last user message for context
  const lastUserMsg = history.filter(m => m.role === 'user').pop()?.content || "";
  let response = basePrompt;

  // Contextual responses based on messageText or history
  if (messageText.toLowerCase().includes("pro plan") || lastUserMsg.toLowerCase().includes("pro plan")) {
