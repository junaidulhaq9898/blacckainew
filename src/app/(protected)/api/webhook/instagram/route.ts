...
export async function POST(req: NextRequest) {
  ...
  // Log the entire webhook payload for debugging (remove in production)
  console.log("Full Webhook Payload:", JSON.stringify(webhook_payload, null, 2));

  let matcher;
  try {
    // Check if the 'messaging' key exists in the payload
    ...
    // Check if the 'changes' key exists in the payload
    ...
    // Ensure matcher contains automationId and is valid
    if (matcher && matcher.automationId) {
      console.log('Matched automationId:', matcher.automationId);

      // Process messaging automation
      ...
      // Handle changes if field is 'comments'
      ...
      console.log('No automationId found or keyword match failed.');

      // Try finding customer history and match automation
